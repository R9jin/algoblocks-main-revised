...
class ComplexityAnalyzer(ast.NodeVisitor):
    def __init__(self, source_code, trace_data=None):
        ...
        self.builtin_complexities = {
            'sort': {'time': 'O(n log n)', 'space': 'O(1)', 'desc': 'Sorts the list in-place using the stable Timsort algorithm. It modifies the original list directly rather than creating a new one.'},
            'sorted': {'time': 'O(n log n)', 'space': 'O(n)', 'desc': 'Creates and returns a completely new sorted list from the items in any iterable, leaving the original data unchanged.'},
            'bisect': {'time': 'O(log n)', 'space': 'O(1)', 'desc': 'Performs a binary search on a sorted sequence to find the rightmost index where a value can be inserted to maintain sorted order.'},
            'bisect_left': {'time': 'O(log n)', 'space': 'O(1)', 'desc': 'Performs a binary search on a sorted sequence to find the leftmost index where a value can be inserted to maintain sorted order.'},
            'bisect_right': {'time': 'O(log n)', 'space': 'O(1)', 'desc': 'Identical to bisect; finds the rightmost insertion point via binary search.'},
            'heappush': {'time': 'O(log n)', 'space': 'O(1)', 'desc': 'Adds a new element to a heap queue. It compares the element to its parent and swaps them up the tree until the min-heap property is restored.'},
            'heappop': {'time': 'O(log n)', 'space': 'O(1)', 'desc': 'Removes and returns the smallest element (the root) from the heap, then shifts elements down the tree to restore the min-heap order.'},
            'heapify': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Rearranges a standard list into a valid min-heap in-place, operating in linear time by trickling items down the tree structure.'},
            'insort': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Finds the correct insertion point using binary search O(log n), but takes linear time O(n) to shift existing elements over to make room.'},
            'insort_left': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Finds the leftmost insertion point via binary search, followed by an O(n) shift of elements to insert the new value.'},
            'insort_right': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Finds the rightmost insertion point via binary search, followed by an O(n) shift of elements to insert the new value.'},
            'gcd': {'time': 'O(log n)', 'space': 'O(1)', 'desc': 'Calculates the greatest common divisor of two integers using the Euclidean algorithm.'},
            'join': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Concatenates an iterable of strings into a single string, using the base string as a separator between each element.'},
            'split': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Scans a string from left to right, dividing it into a list of separate substrings wherever the specified delimiter is found.'},
            'list': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Iterates through an input collection and populates a new list containing all of its elements.'},
            'set': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Iterates through an input collection, hashing each item to populate a new set containing only the unique elements.'},
            'dict': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Creates a new dictionary by iterating over a sequence of key-value pairs and inserting them.'},
            'tuple': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Creates a fixed-size, immutable sequence containing all items from the provided iterable.'},
            'deque': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Initializes a double-ended queue using the elements of an iterable, allowing fast appends and pops from both ends.'},
            'append': {'time': 'O(1) amortized', 'space': 'O(1)', 'desc': 'Adds a single element to the end of a list. Usually instantaneous, but occasionally requires re-sizing the list to make more room.'},
            'insert': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Places an element at a specific index. Every element after that index must be shifted one position to the right.'},
            'max': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Linearly scans through an entire sequence, keeping track of and returning the largest value found.'},
            'min': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Linearly scans through an entire sequence, keeping track of and returning the smallest value found.'},
            'sum': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Adds up the numeric values of an iterable from left to right, keeping a running total.'},
            'any': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Evaluates items sequentially. It stops and returns True immediately upon finding the first "truthy" element.'},
            'all': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Evaluates items sequentially. It stops and returns False immediately upon finding the first "falsy" element.'},
            'len': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Returns the number of items in a container. It simply reads a saved size attribute rather than counting items one by one.'},
            'abs': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Returns the absolute (positive) value of a number by removing its sign.'},
            'round': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Rounds a number to a specified number of decimal places.'},
            'int': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Converts a number or a correctly formatted string into an integer.'},
            'float': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Converts an integer or a correctly formatted string into a floating-point (decimal) number.'},
            'bool': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Evaluates a value to determine its truthiness, returning either True or False.'},
            'type': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Returns the class type that an object belongs to.'},
            'isinstance': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Checks whether an object is an instance of a specific class or derived from it via inheritance.'},
            'str': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Converts an object into its string representation.'},
            'remove': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Searches the list for a specific value and removes its first occurrence, shifting subsequent elements to fill the gap.'},
            'index': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Searches a sequence from left to right to find a target value, returning the index where it first appears.'},
            'count': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Scans the entire collection to count exactly how many times a specific value appears.'},
            'find': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Searches a string for a substring and returns the lowest index where it begins.'},
            'replace': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Creates a new string where occurrences of a target substring are replaced by a new string.'},
            'copy': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Creates a shallow copy of a collection, meaning it builds a new container but fills it with references to the original objects.'},
            'reverse': {'time': 'O(n)', 'space': 'O(1)', 'desc': 'Reverses the order of the items in a list in-place by swapping elements from opposite ends toward the middle.'},
            'extend': {'time': 'O(m)', 'space': 'O(m)', 'desc': 'Takes an iterable of length M and appends all of its elements to the end of the current list.'},
            'upper': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Generates a new string where all alphabetic characters are converted to uppercase.'},
            'lower': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Generates a new string where all alphabetic characters are converted to lowercase.'},
            'strip': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Generates a new string with all specified leading and trailing characters (whitespace by default) removed.'},
            'lstrip': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Generates a new string with all leading characters on the left removed.'},
            'rstrip': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Generates a new string with all trailing characters on the right removed.'},
            'keys': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Returns a dynamic view object showing all the keys currently in the dictionary.'},
            'values': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Returns a dynamic view object showing all the values currently in the dictionary.'},
            'items': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Returns a dynamic view object showing all the key-value pairs currently in the dictionary.'},
            'range': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Creates a lightweight object representing a sequence of numbers, generating them mathematically rather than storing them in memory.'},
            'clear': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Empties the container, removing all of its elements immediately.'},
            'get': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Looks up a key in a dictionary. If the key exists, it returns the value; if not, it safely returns a default fallback.'},
            'popleft': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Removes and returns the first element from the left end of a deque collection.'},
            'union': {'time': 'O(n + m)', 'space': 'O(n + m)', 'desc': 'Combines multiple sets together, creating a new set that contains all unique elements from the participating sets.'},
            'intersection': {'time': 'O(min(n, m))', 'space': 'O(min(n, m))', 'desc': 'Compares sets to create a new set containing only the elements that exist in all of them.'},
            'difference': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Creates a new set with elements from the first set that do not appear in the second set.'},
            'symmetric_difference': {'time': 'O(n)', 'space': 'O(n)', 'desc': 'Creates a new set containing elements that appear in exactly one of the two sets, but not both.'},
            'update': {'time': 'O(m)', 'space': 'O(m)', 'desc': 'Modifies a set in-place by adding all elements found in another provided iterable.'},
            'add': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Hashes a value and inserts it into a set. If the item already exists, the set remains unchanged.'},
            'ord': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Takes a single character string and returns its corresponding Unicode integer value.'},
            'chr': {'time': 'O(1)', 'space': 'O(1)', 'desc': 'Takes an integer and returns its corresponding string character based on Unicode code points.'}
        }
        self.aliases = {}
