...
class TopologicalSequencerMixin:
    """Mixin providing topological function-ordering behavior to ComplexityAnalyzer."""
    ...
    def compute_topological_order(self):
        ...
        """
        known_funcs = list(self.symbol_table.keys())
    ...
    def visit_functions_topologically(self):
        ...
        for name in getattr(self, 'topological_order', []):
            node = self.symbol_table.get(name)
            if node is not None:
                self.visit(node)
