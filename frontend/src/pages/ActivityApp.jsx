...
const getComplexityWeight = (complexity) => {
  ...
  if (comp.includes("n^2") || comp.includes("n²") || comp.includes("n2"))
    return 5;
  if (comp.includes("n^3") || comp.includes("n³") || comp.includes("n3"))
    return 6;
  if (comp.includes("2^n") || comp.includes("2ⁿ") || comp.includes("2n"))
    return 7;
...
const ActivityAppInner = ({ moduleId, activityId }) => {
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      setIsOnline(false);
      showToast("Connection lost. Saving drafts locally.", "error");
    };
  ...
  workerMessageHandler.current = (event) => {
    ...
    if (type === "ANALYZE_RESULT") {
      if (data.status === "success") {
        setAnalysisTime(
          data.analysis_time_ms ? data.analysis_time_ms.toFixed(2) : "0.00",
        );
        setAnalysisResult({
          total: data.total,
      ...
      if (testResolveRef.current) {
        setTimeout(() => {
          ...
          outputAccumulatorRef.current +=
            flushed + (data !== undefined && data !== null ? data : "");
          if (counts)
            setLineExecutions((prev) => {
              const next = { ...prev };
              Object.keys(counts).forEach(
                (k) => (next[k] = Math.max(next[k] || 0, counts[k])),
              );
              return next;
            });
          if (testResolveRef.current)
  ...
  const fetchJsonWithCache = async (cacheKey, url) => {
    try {
      ...
      console.warn(
        `Network fetch failed for ${url}, falling back to cache.`,
        e,
      );
    }
    try {
  ...
  const resolveActivityFromModule = async () => {
    ...
    const activitiesJson = await fetchJsonWithCache(
      `activities:module_${mid}`,
      activitiesUrl,
    );
    ...
    return {
      ...
      type:
        foundActivity.type ||
        (foundLessonKey === "optimizations" ? "optimization" : "activity"),
      difficulty:
        foundActivity.difficulty ||
        (foundLessonKey === "optimizations" ? "Advanced" : "Easy"),
      targetTimeComplexity:
        foundActivity.targetTime ||
        foundActivity.targetTimeComplexity ||
        "O(n)",
      targetSpaceComplexity:
        foundActivity.targetSpace ||
        foundActivity.targetSpaceComplexity ||
        "O(n)",
      testCasesList,
  ...
  const triggerFinalSave = () => {
    ...
    if (
      state.pythonCode === "# Drag blocks to generate Python code" &&
    ...
    if (navigator.onLine && API_BASE) {
      try {
        ...
        syncQueueDB.setItem(`sync_${finalSubId}`, {
          type: "SUBMISSION",
          action: "UPSERT",
          data: payload,
        });
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        ...
        latestStateRef.current.targetTime =
          resolvedActivity.targetTimeComplexity;
        latestStateRef.current.targetSpace =
          resolvedActivity.targetSpaceComplexity;
        ...
        try {
          localSubmission = await submissionsDB.getItem(submissionId);
        } catch (e) {}
        ...
        if (navigator.onLine && !user.isGuest && API_BASE) {
          try {
            ...
            const res = await fetch(
              `${API_BASE}/api/get-submission?activityId=${activityId}&moduleId=${moduleId}`,
              {
        ...
        const isLocalBlank =
          !localCode || localCode === "# Drag blocks to generate Python code";
        const isCloudBlank =
          !cloudCode || cloudCode === "# Drag blocks to generate Python code";
        ...
        if (isLocalBlank && !isCloudBlank) {
          ...
          if (
            (localSubmission.timestamp || 0) >= (cloudSubmission.timestamp || 0)
          )
        ...
        if (
          ...
          finalSubmissionToLoad.activityId === activityId &&
          !cancelled
        ) {
          try {
            const json =
              finalSubmissionToLoad.workspace?.blocklyJson ||
              finalSubmissionToLoad.blocklyJson ||
              {};
            ...
            latestStateRef.current.passed =
              finalSubmissionToLoad.passedTestCases ||
              finalSubmissionToLoad.passed_tests ||
              0;
            latestStateRef.current.status =
          ...
          if (resolvedActivity.templateUrl) {
            try {
              const templateJson = await fetchJsonWithCache(
                `template:${resolvedActivity.id}`,
                resolvedActivity.templateUrl,
              );
        ...
        const savedTests = localStorage.getItem(
          `activity_tests_${moduleId}_${activityId}`,
        );
        if (savedTests && !cancelled) {
          try {
            const { consoleOutput: savedOut, passedTests: savedPassed } =
  ...
  const saveSubmission = async (
    ...
    const finalPassed =
      passed !== null ? passed : latestStateRef.current.passed;
    const finalTestResults =
      testResults !== null ? testResults : latestStateRef.current.testResults;
    ...
    if (!navigator.onLine) {
      await syncQueueDB.setItem(`sync_${submissionId}_${Date.now()}`, {
        type: "SUBMISSION",
        action: "UPSERT",
        data: payload,
      });
    ...
    try {
      ...
      if (API_BASE) {
        ...
        if (response.ok) {
          await submissionsDB.setItem(submissionId, {
            ...payload,
            isSynced: true,
          });
        } else {
    ...
    saveDraftTimeoutRef.current = setTimeout(async () => {
      if (
        ...
        await saveSubmission(
          ...
          totalTests,
          null,
          latestStateRef.current.actualTime,
          latestStateRef.current.actualSpace,
          true,
        );
      }
    }, 1500);
  ...
  useEffect(() => {
    ...
    if (
      ...
      isEngineReady &&
      workerRef.current &&
      generatedPython !== "# Drag blocks to generate Python code" &&
      isEditingCode
    ) {
  ...
  const savePartialProgress = async (lessonId, score) => {
    ...
    await progressDB.setItem(lessonId, {
      score: user.progress[lessonId],
      isSynced: false,
    });
    ...
    if (navigator.onLine && !user.isGuest && API_BASE) {
      try {
        ...
        console.error("Progress update to backend failed:", error);
  ...
  const completeFullTopic = async (topicId) => {
    ...
    await progressDB.setItem(topicId, {
      score: 100,
      completed: true,
      isSynced: false,
    });
  ...
  const executeTest = async (codeToRun) => {
    return new Promise((resolve, reject) => {
      ...
      runTimeoutRef.current = setTimeout(() => {
        resetWorker();
        if (testRejectRef.current) {
          testRejectRef.current(
            new Error(
              "Infinite Loop detected. Execution timed out after 10 seconds.",
            ),
          );
          testResolveRef.current = null;
          testRejectRef.current = null;
  ...
  const runTestCases = async () => {
    ...
    setLineExecutions({});
    setConsoleOutput(
      "Running pre-flight checks (Detecting infinite loops)...\n",
    );
    setBottomPanel("console");
    setConsoleTab("output");
    ...
    try {
      ...
      localStorage.setItem(
        `activity_tests_${moduleId}_${activityId}`,
        JSON.stringify({ consoleOutput: errorMsg, passedTests: 0 }),
      );
      return;
    }
    ...
    for (let i = 0; i < totalTests; i++) {
      ...
      if (tc.isComplexityTest) {
        const actualVal = tc.title.includes("Time")
    ...
    if (functionalPassed === 0) score = 0;
      ...
      score = 3;
      const targetTimeWeight = getComplexityWeight(
        activityDataResolved?.targetTimeComplexity || "O(n)",
      );
      const targetSpaceWeight = getComplexityWeight(
        activityDataResolved?.targetSpaceComplexity || "O(n)",
      );
      const actualTimeWeight = getComplexityWeight(
        analysisResult.total || "O(n^2)",
      );
      const actualSpaceWeight = getComplexityWeight(
        analysisResult.space_total || "O(n)",
      );
    ...
    const testResults = processedTestCases.map((tc, idx) => ({
      id: `tc_${idx}`,
      status: fullOutput.includes(`Test ${idx + 1}: PASSED`)
        ? "passed"
        : "failed",
    }));
    ...
    <div className="activity-app-container">
      ...
      <header className="activity-topbar">
        ...
        >
          ...
          <button
            className="activity-action-btn run-btn"
      ...
      <Split
        className={`activity-main-layout ${!isLeftPanelVisible ? "left-hidden" : ""} ${!isRightPanelVisible ? "right-hidden" : ""}`}
        sizes={[25, 50, 25]}
        minSize={[
          isLeftPanelVisible ? 250 : 0,
          400,
          isRightPanelVisible ? 250 : 0,
        ]}
        gutterSize={8}
      >
        <aside className="activity-left-panel">
          {lessonActivitiesResolved.length > 0 && (
            <div className="activity-selector-container">
              ...
              <select
                ...
                value={activityId}
                onChange={(e) =>
                  navigate(`/activity/${moduleId}/${e.target.value}`)
                }
              >
          ...
          <div className="activity-panel-content">
            ...
            >
              {renderFormattedTask(
                activityDataResolved?.task || "Loading activity...",
              )}
            </div>
          </div>
        </aside>
        ...
        <main className="workspace-main activity-center-panel">
          <button
            ...
            title={
              isLeftPanelVisible ? "Hide Instructions" : "Show Instructions"
            }
          >
          ...
          <button
            className={`sidebar-toggle-btn right-panel-toggle ${!isRightPanelVisible ? "closed" : ""}`}
            onClick={() => setIsRightPanelVisible(!isRightPanelVisible)}
            title={isRightPanelVisible ? "Hide Test Cases" : "Show Test Cases"}
          >
            <span className="toggle-icon">
              {isRightPanelVisible ? ">" : "<"}
            </span>
          ...
          >
            ...
            >
              ...
              >
                <span
                  className="python-sync-status"
                  style={{ color: "#EBE4FF", fontSize: "0.85rem" }}
                >
                  {isEditingCode
                    ...
                    {activeTab === "memory" ? (
                      ...
                      >
                        <MemoryVisualizer
                          ...
                          currentStep={
                            analysisResult.lines.length > 0
                              ? analysisResult.lines.length - 1
                              : 0
                          }
                        />
                      </div>
                          ...
                          <tbody>
                            {analysisResult.lines.map((line, i) => {
                              ...
                              let spaceExp =
                                ...
                                "Analysis not available.";
                              const isBottleneck =
                                actualBottleneckIndices.includes(i);
                              const timeColor =
                ...
                onClick={() =>
                  setModalConfig({
                    ...
                    onConfirmAction: async () => {
                      ...
                      if (storedUser) {
                        const user = JSON.parse(storedUser);
                        await submissionsDB.removeItem(
                          `${user.email}_${moduleId}_${activityId}`,
                        );
                      }
        ...
        <aside className="activity-right-panel">
          ...
          <div className="activity-panel-content">
            {processedTestCases.map((tc, i) => {
              ...
              const isPassing = consoleOutput.includes(
                `${testIdentifier}: PASSED`,
              );
              const isFailing = consoleOutput.includes(
                `${testIdentifier}: FAILED`,
              );
              const isError = consoleOutput.includes(
                `${testIdentifier}: ERROR`,
              );
              ...
              const statusClass = isPassing
                ...
                : isFailing || isError
                  ? "failing"
                  : "";
              const displayTitle = tc.isComplexityTest
                ? tc.title
                : tc.isHidden
                  ? `Hidden Test`
                  : `Test ${i + 1}`;
                ...
                <div key={i} className={`test-case-card ${statusClass}`}>
                  ...
                  >
                    <div className="test-case-header-left">
                      <div
                        className={`test-case-indicator ${statusClass}`}
                      ></div>
                    ...
                    {tc.isHidden ? (
                      ...
                      <span
                        className={`test-case-chevron ${isExpanded ? "open" : ""}`}
                      >
                        v
                      </span>
                    )}
