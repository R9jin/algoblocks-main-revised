...
export default function LessonViewer() {
      ...
      for (const module of curriculumIndex) {
        for (const lessonMeta of module.lessons) {
          try {
            ...
            console.error(
              `Failed to fetch lesson data for ${lessonMeta.lessonId}`,
              e
            );
          }
        }
  ...
  useEffect(() => {
    const loadLesson = async () => {
      ...
      try {
        const module = curriculumIndex.find(
  ...
  const buildLockMap = () => {
    ...
    for (const module of curriculumIndex) {
      const preComplete = hasPreAssessment(
        module.moduleId
      );
  ...
  const currentModule = curriculumIndex.find(
    (m) => m.moduleId === moduleId
  );
      ...
      <aside className="lesson-modules-sidebar">
        ...
        <nav className="modules-nav">
          {curriculumIndex.map((module) => {
            ...
            const modComplete = isModuleComplete(
              module.moduleId
            );
                ...
                {isExpanded && (
                  ...
                  >
                    ...
                    <div
                      onClick={() =>
                        navigate(
                          `/assessment/${module.moduleId}/pre`
                        )
                      }
                      style={{
                        display: "flex",
                    ...
                    <div
                      onClick={() => {
                        if (
                          ...
                          navigate(
                            `/assessment/${module.moduleId}/post`
                          );
                        }
                      }}
                      style={{
      ...
      <main className="lesson-viewer-container">
        {isCurrentLessonLocked ? (
          ...
          >
            ...
            <p>
              Please complete the pre-assessment and
              preceding activities to unlock this lesson.
            </p>
              ...
              {currentActivityId && (
                ...
                >
                  ...
                  >
                    ...
                    an interactive coding activity.
                  </p>

                  <button
                    onClick={() =>
                      navigate(
                        `/activity/${moduleId}/${currentActivityId}`
                      )
                    }
                    style={{
                      padding: "14px 28px",
