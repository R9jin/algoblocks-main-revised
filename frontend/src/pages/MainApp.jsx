...
export default function MainApp() {
  ...
  // project/template before any of the earlier ones registers as "loaded"
  // in the tab -- this is how an account could end up with far more than
  // the intended 20 saved items.
  ...
  const workspaceTour = {
    ...
    steps: [
      ...
      { target: ".complexity-content .tab-btn-group .tab-btn:nth-child(2)", title: "Inspect complexity", description: "Review the per-line time and space cost as the analysis flows through the code.", onEnter: () => { focusDockPanel("complexity"); setActiveComplexityTab("complexity"); } },
      { target: ".complexity-content .tab-btn-group .tab-btn:nth-child(3)", title: "Inspect memory map", description: "See how memory changes while the code executes.", onEnter: () => { focusDockPanel("complexity"); setActiveComplexityTab("memory"); } },
      { target: ".complexity-content .tab-btn-group .tab-btn:nth-child(4)", title: "Inspect the call graph", description: "Trace recursive calls and control flow through the call graph.", onEnter: () => { focusDockPanel("complexity"); setActiveComplexityTab("callgraph"); } },
  ...
  const submitSave = async () => {
    // Re-entrancy guard: ignore extra clicks/keypresses while a save is
    // already in flight instead of letting them fire another concurrent
    // request (see isSavingModal declaration for why that matters).
        ...
        <div className="modal-overlay">
          <div className="save-modal-content">
            ...
            <div className="save-modal-actions">
              <button onClick={() => setSaveModal({ ...saveModal, isOpen: false })} disabled={isSavingModal} className="save-modal-cancel-btn">Cancel</button>
              <button onClick={submitSave} disabled={isSavingModal} className="save-modal-confirm-btn">{isSavingModal ? "Saving..." : "Save"}</button>
            </div>
          </div>
        </div>
      )}
